...
public class circleExplanListView extends ExpandableListView implements
		OnScrollListener {
	...
	public circleExplanListView(Context context, ArrayList<String> dataList) {
		this(context);
		this.data = dataList;
	}

	public circleExplanListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		// TODO Auto-generated constructor stub
		init(context);
	}

	public circleExplanListView(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
		init(context);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			Log.i("headView.getBottom()",
					headViewHeight + ",,," + headView.getBottom());
			if (deltaCount > 0 && currentState != LOADSTATE_LOADING
					&& firstVisibleItem == 0
					&& headView.getBottom() >= headViewHeight) {
				Log.i("headView.getBottom()", headViewHeight + "");
				if (mListViewListener != null)
					mListViewListener.onRefresh();
				decreasePadding(deltaCount);
				// loadDataForThreeSecond();
