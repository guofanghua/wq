...
public class noticeListAdapter extends BaseExpandableListAdapter {
	ArrayList<noticeList> list = null;
	Context context;
	...
	public noticeListAdapter(Context context, ArrayList<noticeList> list) {
	...
	public View getChildView(final int groupPosition, final int childPosition,
		...
		final notice item = list.get(groupPosition).getList()
				.get(childPosition);
		if (convertView == null) {
		...
		if (item.getPic().size() == 1) {
			...
			String url = item.getPic().get(0).getImageUrl();
			if (item.getPic().get(0).getFlag() == 1) {
		...
		if (item.getPic().size() == 2) {
			...
			for (int i = 0; i < item.getPic().size(); i++) {
				LayoutParams params = new LayoutParams(DensityUtil.dip2px(
						context, 44), height);
				params.leftMargin = 0;
				photoModel p = item.getPic().get(i);
		...
		if (item.getPic().size() >= 1) {
			...
			viewHolderChild.txt_picCount.setText(String.format(context
					.getString(R.string.find_ec_detail_count), item.getPic()
					.size()));
		} else {
